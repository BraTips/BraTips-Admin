...
const customizerUserName = computed(() => authStore.user?.name || authStore.user?.email || 'Administrator');
</script>
      ...
      <template v-slot:activator="{ props }">
        <v-btn class="profileBtn" variant="text" rounded="sm" v-bind="props">
          <div class="d-flex align-center">
            ...
            <h6 class="text-subtitle-1 mb-0 d-sm-block d-none">{{ customizerUserName }}</h6>
          </div>
        </v-btn>
      </template>
